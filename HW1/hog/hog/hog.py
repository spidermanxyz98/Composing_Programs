...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls > 0:
        cur_score = roll_dice(num_rolls)
    else:
        #print('free bacon')
        digit1, digit2 = opponent_score // 10, opponent_score % 10
        cur_score = 1 + max(digit1, digit2)
    return cur_score
# Playing a game
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        ...
        # Assume I am player0 and the opponent is player1
        if who == 0:
        ...
        if score == opponent_score * 2 or opponent_score == score * 2:
           # print('Swine swap')
           # print('score: {}, opponent score: {}'.format(score, opponent_score))
           # t = score   # Swine swap
           # score = opponent_score
           # opponent_score = t
            score, opponent_score = opponent_score, score
           # print('after swap, score: {}, opponent_score: {}'.format(score, opponent_score))
        who = other(who)    # swith turn
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for num_rolls in range(1, 11):
        average_score = make_averaged(roll_dice, 1000)(num_rolls, dice)
        print('{} dice scores {} on average'.format(num_rolls, average_score))
        if average_score > cur_max_score:
            cur_max_score = average_score
            cur_max_rolls = num_rolls

    print('optimized number of rolls {}'.format(cur_max_rolls))
    return cur_max_rolls
...
def bacon_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    factor, residual = opponent_score // 10, opponent_score % 10

    if max(factor, residual) + 1 >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    factor, residual = opponent_score // 10, opponent_score % 10
    free_bacon = max(factor, residual) + 1

    if (score + free_bacon) * 2 == opponent_score:
        return 0
    if free_bacon >= BACON_MARGIN and opponent_score * 2 != score + free_bacon:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    "*** YOUR CODE HERE ***"
    # varay rolls based on dice

    if (score + opponent_score) % 7 == 0:
        base_line = 4
    else:
        base_line = BASELINE_NUM_ROLLS

    factor, residual = opponent_score // 10, opponent_score % 10
    free_bacon = max(factor, residual) + 1

    if (score + free_bacon) * 2 == opponent_score or free_bacon > BACON_MARGIN:
        return 0
